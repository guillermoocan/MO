

## {.title-slide-custom}

<div class="cover">

<div class="cover-logo">
<img src="Assets/logo_big.png">
</div>


<div class="cover-title">
Introducción
</div>

<div class="cover-line"></div>

<div class="cover-subtitle">
Capítulo 1
</div>

</div>

<div class="menu-title" style="display:none;">Introducción </div>

---

::: {.center-vertical}

::: {.chapter-title}

Introducción

:::

<div class="chapter-line"></div>

::: {.chapter-subtitle}

Definiciones

<div class="menu-title" style="display:none;"> ▸ Definiciones </div>

:::

:::



## Mecánica


::: {.columns}

::: {.column}

<div class="vspace-3"></div>
<div class="vspace-3"></div>


:::{.definition-block .fragment}

**Definición**

Rama de la física que estudia el movimiento y el equilibrio de los cuerpos, así como las fuerzas y las interacciones que determinan su estado de movimiento.
:::

<div class="vspace-3"></div>

:::{.incremental}

- La mecánica responde dos preguntas fundamentales sobre el movimiento de los cuerpos: **¿Cómo y por qué se mueve un cuerpo?**

- La **cinemática** responde al *cómo*: describe la posición, velocidad y aceleración de los cuerpos sin considerar las causas que producen su movimiento.

- La **dinámica** responde al *por qué*: estudia las causas del movimiento y relaciona el movimiento de los cuerpos con las fuerzas que actúan sobre ellos.

- La **estática**, por su parte, estudia las condiciones de **equilibrio** de los cuerpos, es decir, las situaciones en las que la resultante de fuerzas es compatible con un movimiento nulo aparente. 
:::

:::

::: {.column}

![](/Figures/c1/definition_mecanica.svg){width=70% fig-align="center"}

:::

:::



## Astronomía

::: {.columns}

::: {.column}

<div class="vspace-3"></div>
<div class="vspace-3"></div>

:::{.definition-block .fragment}

**Definición**

Rama de la ciencia que estudia los cuerpos celestes, los fenómenos y procesos que ocurren en el universo, así como su origen, evolución, estructura y movimiento.
:::

<div class="vspace-3"></div>

:::{.incremental}

- La astronomía busca responder preguntas fundamentales sobre el universo: **¿qué existe, cómo se comporta y cómo evoluciona?**

<!-- - La **astronomía observacional** estudia los objetos y fenómenos celestes mediante la adquisición y análisis de radiación y otras señales provenientes del espacio.

- La **astronomía teórica** desarrolla modelos físicos y matemáticos para explicar y predecir los fenómenos observados. -->

- La **astrofísica** aplica las leyes de la física para estudiar las propiedades y procesos físicos de los cuerpos y sistemas astronómicos.

<!-- - La **cosmología** estudia el origen, evolución, estructura y destino del universo como un todo. -->
:::

:::

::: {.column}

![](/Figures/c1/definition_astronomia.svg){width=70% fig-align="center"}

:::

:::



## Mecánica celeste

::: {.columns}

::: {.column}

<div class="vspace-3"></div>
<div class="vspace-3"></div>

:::{.definition-block .fragment}

**Definición**

Rama de la mecánica que estudia el movimiento de los cuerpos celestes y las interacciones gravitacionales que determinan su evolución dinámica.
:::

<div class="vspace-3"></div>

:::{.incremental}

- Se fundamenta principalmente en las **leyes del movimiento de Newton** y la **ley de gravitación universal**.

- Estudia el movimiento de **planetas, lunas, asteroides, cometas, estrellas** y otros cuerpos celestes.

- Considera tanto sistemas simples como sistemas formados por **múltiples cuerpos que interactúan gravitacionalmente**.

- Permite describir y predecir fenómenos como **órbitas, eclipses, encuentros entre cuerpos y perturbaciones gravitacionales**.

:::

:::

::: {.column}

![](/Figures/c1/definition_celeste.svg){width=70% fig-align="center"}

:::

:::


## Astrodinámica

::: {.columns}

::: {.column}

<div class="vspace-3"></div>
<div class="vspace-3"></div>

:::{.definition-block .fragment}

**Definición**

Estudio del movimiento de objetos hechos por el ser humano en el espacio, sujetos tanto a fuerzas naturales como a fuerzas inducidas artificialmente.
:::

<div class="vspace-3"></div>

:::{.incremental}

- Contempla tanto el movimiento **traslacional** como el movimiento **rotacional** de los objetos. 

- Las **fuerzas naturales** incluyen principalmente la gravedad, así como otras perturbaciones producidas por el entorno espacial.

- Las **fuerzas artificialmente inducidas** son aquellas generadas deliberadamente para modificar el movimiento del vehículo, como las producidas por **sistemas de propulsión**.

- La astrodinámica permite **analizar, predecir y diseñar** el movimiento de vehículos espaciales.


:::

:::

::: {.column}

![](/Figures/c1/definition_astrodinamica.svg){width=70% fig-align="center"}

:::

:::

## Mecánica orbital

::: {.columns}

::: {.column}

<div class="vspace-3"></div>
<div class="vspace-3"></div>

:::{.definition-block .fragment}

**Definición**

Estudio del movimiento *traslacional* de objetos hechos por el ser humano en el espacio, sujetos tanto a fuerzas naturales como a fuerzas inducidas artificialmente.
:::

<div class="vspace-3"></div>

:::{.incremental}

- Estudia las **trayectorias traslacionales** de cuerpos naturales con respecto de una referencia.

- A las trayectorias periódicas se les conoce como **órbitas**.  

- Nos permite determinar y predecir la **posición y velocidad** de un cuerpo a lo largo de su trayectoria.

- Analiza cómo las **fuerzas gravitacionales y perturbaciones** modifican el movimiento orbital.


:::

:::

::: {.column}

![](/Figures/c1/definition_orbital.svg){width=70% fig-align="center"}

:::

:::


## Satélites

:::{.definition-block}

**Satélite**

Cuerpo u objeto que se encuentra en órbita alrededor de otro cuerpo debido a la interacción gravitacional entre ellos.
:::

::: {.columns}

::: {.column .fragment}

![](/Figures/c1/definition_sat_nat.png){.fragment width=52% fig-align="center"}

:::{.definition-block}

**Satélite natural**

Cuerpo celeste de origen natural que orbita alrededor de otro cuerpo celeste como consecuencia de su interacción gravitacional.
:::

:::

::: {.column .fragment}

![](/Figures/c1/definition_sat_art.png){.fragment width=100% fig-align="center"}

:::{.definition-block}

**Satélite artificial**

Objeto construido por el ser humano y colocado en órbita alrededor de un cuerpo celeste para cumplir una función determinada.

:::

:::

:::



## Importancia en la ingeniería

- El diseño de un **vehículo espacial** no puede realizarse de manera independiente de la **misión espacial** que debe cumplir.

<div class="vspace-3"></div>


::: {.definition-block .fragment}

**Misión espacial**

Conjunto de actividades, sistemas y operaciones planificadas para alcanzar uno o más objetivos en el espacio, desde su lanzamiento hasta la finalización de la misión.
:::

<div class="vspace-3"></div>

::: {.incremental}

- Antes de diseñar completamente el vehículo es necesario establecer **qué se quiere lograr, dónde debe operar y cómo llegará hasta allí**.

- Todas las actividades de la misión deben ser pensadas alrededor de la **carga útil** del satélite. 
  
- La misión determina requisitos fundamentales del vehículo, como **propulsión, energía, comunicaciones, navegación y estructura**.

- Dentro del diseño de la misión, la **trayectoria** determina cómo se desplazará el vehículo desde su estado inicial hasta alcanzar las condiciones requeridas para cumplir el objetivo.

- Por ello, el diseño y análisis de trayectorias constituye un elemento **esencial para el diseño de una misión espacial**.

:::


## Subsistemas de un satélite


::: {.fragment}

![](Figures/c1/subsystems.svg){width=70% fig-align="center"}

:::



##

El diseño de la trayectoria **se materializa en la práctica** en diferentes sistemas y operaciones del vehículo espacial.

::: {.columns}

::: {.column}

:::{.definition-block .fragment}

**Despliegue**

La trayectoria determina las condiciones y el momento en que el vehículo debe ser liberado, separado o desplegado de su vehículo lanzador o de otro sistema espacial.
:::

![](/Figures/c1/definition_ppod.png){ width=45% fig-align="center"}

:::{.definition-block .fragment}

**Computadora de a bordo**

Los modelos y algoritmos de propagación, control y determinación orbital permiten estimar la posición y velocidad del vehículo durante la misión.
:::

![](/Figures/c1/definition_obc.png){width=43% fig-align="center"}

:::

::: {.column}

:::{.definition-block .fragment}

**Sistema de propulsión**

Las maniobras diseñadas para modificar y controlar la trayectoria determinan los requisitos de impulso, dirección y momento de las actuaciones propulsivas.
:::

![](/Figures/c1/definition_thrusters.png){width=43% fig-align="center"}

:::{.definition-block .fragment}

**Sistema de comunicaciones**

La trayectoria determina la visibilidad del vehículo respecto a las estaciones terrestres, estableciendo condiciones para la planificación de enlaces, ventanas de comunicación y orientación de las antenas.

:::

![](/Figures/c1/definition_ant.png){ width=45% fig-align="center"}

:::

:::




---

::: {.center-vertical}

::: {.chapter-title}

Introducción

:::

<div class="chapter-line"></div>

::: {.chapter-subtitle}

Repaso de álgebra

<div class="menu-title" style="display:none;"> ▸ Repaso de álgebra </div>

:::

:::


## Vectores


::: {.columns}
::: {.column width="70%"}

Un **vector** (columna) es una $n$-tupla ordenada de números reales:

$$
    \vec{v}=
    \begin{bmatrix}
        v_1\\
        v_2\\
        \vdots\\
        v_n
    \end{bmatrix} \in\mathbb{R}^n ,
    \qquad v_i\in\mathbb{R}
$$

o de forma equivalente,

$$
    \vec{v}\equiv
    [v_1\quad v_2\quad \dots\quad v_n]^\top.
$$

donde $n\in\mathbb{N}$ y $\top$ denota transposición. El conjunto de todos los vectores de dimensión $n$ es el espacio $\mathbb{R}^n$.

Físicamente nos permiten representar **magnitudes vectoriales**, *e.g.*, la posición y la velocidad.

Pueden clasificarse como libres o fijos, dependiendo si origen es invariante.

:::
::: {.column width="30%"}

:::{.small}

$v_x$
```{ojs}
viewof vx = Inputs.range([-5, 5], {value: 1, step: 0.1})
```
$v_y$
```{ojs}
viewof vy = Inputs.range([-5, 5], {value: 1, step: 0.1})
```

$o_x$
```{ojs}
viewof ox = Inputs.range([-5, 5], {value: 0, step: 0.1})
```
$o_y$
```{ojs}
viewof oy = Inputs.range([-5, 5], {value: 0, step: 0.1})
```

:::

```{ojs}
Plot.plot({
  width: 500,
  height: 500,
  x: {domain: [-5, 5], grid: true},
  y: {domain: [-5, 5], grid: true},
  marks: [
    Plot.arrow(
      [{x1: ox, y1: oy, x2: vx+ox, y2: vy+oy}],
      {
        x1: "x1",
        y1: "y1",
        x2: "x2",
        y2: "y2"
      }
    )
  ]
})
```
:::

:::

---

## Operaciones típicas entre vectores

Sean $\vec{u},\vec{v}\in\mathbb{R}^n$ y $\alpha\in\mathbb{R}$:

- **Suma:** $\vec{u}+\vec{v}=[u_1+v_1,\dots,u_n+v_n]^\top$

- **Producto por escalar:** $\alpha\vec{u}=[\alpha u_1,\dots,\alpha u_n]^\top$

- **Producto punto:**

$$
    \begin{aligned}
        \vec{u}\cdot\vec{v}&=\sum_{i=1}^{n}u_iv_i\\
        &=\vec{u}^{\top}\vec{v}\\
        &=\|\vec{u}\|\,\|\vec{v}\|\cos\theta
    \end{aligned}
$$

donde $\theta$ es el ángulo comprendido entre $\vec{u}$ y $\vec{v}$ al hacer coincidir sus orígenes.

- **Norma Euclidiana:** $\|\vec{u}\|=\sqrt{\vec{u}\cdot\vec{u}}$

---

## Producto cruz (definición por componentes)

En $\mathbb{R}^3$, el **producto cruz** de dos vectores $\vec{u},\vec{v}\in\mathbb{R}^3$ se define como

$$
    \vec{u}\times\vec{v}=
    \begin{bmatrix}
        u_2v_3-u_3v_2\\
        u_3v_1-u_1v_3\\
        u_1v_2-u_2v_1
    \end{bmatrix}=
    \begin{vmatrix}
        \vec{i} & \vec{j} & \vec{k}\\
        u_1 & u_2 & u_3\\
        v_1 & v_2 & v_3
    \end{vmatrix},
$$

donde $\{\vec{i},\vec{j},\vec{k}\}$ es la base canónica de $\mathbb{R}^3$.

::: {.fragment}

**Propiedades**:

- $\vec{u}\times\vec{v}=-(\vec{v}\times\vec{u})$
- $\vec{u}\times\vec{u}=\vec{0}$
- $\vec{u}\cdot(\vec{u}\times\vec{v})=0$
- $\vec{u}\times\vec{v}$ es ortogonal (perpendicular) a $\vec{u}$ y $\vec{v}$.
- Si $\vec{u}$ y $\vec{v}$ son colineales, entonces $\vec{u}\times\vec{v}=\vec{0}$.
- $\|\vec{u}\times\vec{v}\|=\|\vec{u}\|\,\|\vec{v}\|\sin\theta.$

:::

---

## Espacio vectorial (real)

Un **espacio vectorial real** es una tupla

$$
(\mathcal{V},+,\cdot)
$$

donde $\mathcal{V}$ es un conjunto de vectores reales, $+:\mathcal{V}\times\mathcal{V}\rightarrow\mathcal{V}$ y $\cdot:\mathbb{R}\times\mathcal{V}\rightarrow\mathcal{V}$ satisfacen, para todo $\vec{u},\vec{v},\vec{w}\in\mathcal{V}$ y $\alpha,\beta\in\mathbb{R}$:

- $\alpha(\vec{u}+\vec{v})=\alpha\vec{u}+\alpha\vec{v}$.
- $(\alpha+\beta)\vec{u}=\alpha\vec{u}+\beta\vec{u}$.
- $(\alpha\beta)\vec{u}=\alpha(\beta\vec{u})$.
- $1\cdot\vec{u}=\vec{u}$.

---

## Base

Sea $\mathcal{V}$ un espacio vectorial real de dimensión $n$. Un conjunto ordenado

$$
\mathcal{B}=\{\vec{v}_1,\vec{v}_2,\dots,\vec{v}_n\}\subset\mathcal{V}
$$

es una **base** de $\mathcal{V}$ si:

1. **Independencia lineal**

$$
    \alpha_1\vec{v}_1+\cdots+\alpha_n\vec{v}_n=\vec{0}
    \;\Rightarrow\;
    \alpha_1=\cdots=\alpha_n=0.
$$


::: {.fragment}

1. **Generación**

Todo $\vec{u}\in\mathcal{V}$ se escribe de forma *única* como

$$
    \vec{u}=\alpha_1\vec{v}_1+\cdots+\alpha_n\vec{v}_n,
    \qquad \alpha_i\in\mathbb{R}.
$$

:::



::: {.callout-note .fragment}
### Nota:

 Una *combinación lineal* es toda expresión

$$
\alpha_1\vec{v}_1+\cdots+\alpha_n\vec{v}_n
$$

con $\alpha_i\in\mathbb{R}$. La independencia lineal de la base implica que ningún vector se expresa como combinación lineal de los demás.

:::

---

## Matrices

Una **matriz** $A\in\mathbb{R}^{m\times n}$ es un arreglo rectangular:

$$
    A=
    \begin{bmatrix}
        a_{11} & \cdots & a_{1n}\\
        \vdots & \ddots & \vdots\\
        a_{m1} & \cdots & a_{mn}
    \end{bmatrix}.
$$

::: {.fragment}

**Operaciones con matrices**

Sean $A,B\in\mathbb{R}^{m\times n}$ y $\alpha\in\mathbb{R}$:

- **Suma:** $(A+B)_{ij}=a_{ij}+b_{ij}$.
- **Producto por escalar:** $(\alpha A)_{ij}=\alpha a_{ij}$.
- **Producto matricial:** Si $A\in\mathbb{R}^{m\times p}$ y $B\in\mathbb{R}^{p\times n}$,

$$
    (AB)_{ij}=\sum_{k=1}^{p}a_{ik}b_{kj},
    \qquad
    AB\in\mathbb{R}^{m\times n}.
$$

- **Transpuesta:** $(A^\top)_{ij}=a_{ji}$.

**Nota:** La notación $(A)_{ij}$ denota el elemento de $A$ en la fila $i$ y columna $j$.

:::


## Inversa de una matriz

Sea $A\in\mathbb{R}^{n\times n}$ (matriz cuadrada). Se dice que $A$ es **invertible** si existe $B\in\mathbb{R}^{n\times n}$ tal que

$$
AB=BA=I_n.
$$

Dicha $B$ es única y se denota $A^{-1}$.

Equivalencias:

- $A$ invertible $\iff \det(A)\neq0 \iff$ las columnas de $A$ son linealmente independientes.
- $A$ invertible $\iff \operatorname{rank}(A)=n$.

::: {.callout-note}
### Nota:

El determinante de una matriz cuadrada es un escalar asociado que puede interpretarse como una combinación alternada de todos los posibles productos de sus elementos. Por otro lado, el rango de una matriz es el número de sus columnas linealmente independientes. En el caso de matrices cuadradas, el rango de $A$ es $n$ si y solo si $\det(A)\neq0$.

:::

---

## Matrices simétricas y antisimétricas

Sea $A\in\mathbb{R}^{n\times n}$.

- $A$ es **simétrica** si

$$
A^\top=A.
$$

Ejemplo:

$$
    A=
    \begin{bmatrix}
        a & b\\
        b & c
    \end{bmatrix}.
$$

- $A$ es **antisimétrica** si

$$
A^\top=-A.
$$

Ejemplo:

$$
    A=
    \begin{bmatrix}
        0 & -a\\
        a & 0
    \end{bmatrix}.
$$

---

## Producto cruz: matriz antisimétrica

Dado $\vec{u}=[u_1,u_2,u_3]^\top$, se define

$$
S(\vec{u})=
\begin{bmatrix}
0 & -u_3 & u_2\\
u_3 & 0 & -u_1\\
-u_2 & u_1 & 0
\end{bmatrix},
\qquad
\vec{u}\times\vec{v}=S(\vec{u})\vec{v}.
$$

::: {.fragment}

**Ejemplo**

$$
\vec{u}=
\begin{bmatrix}
2\\-1\\3
\end{bmatrix},
\qquad
\vec{v}=
\begin{bmatrix}
1\\4\\2
\end{bmatrix},
$$

$$
S(\vec{u})=
\begin{bmatrix}
0&-3&-1\\
3&0&-2\\
1&2&0
\end{bmatrix},
\qquad
S(\vec{u})\vec{v}=
\begin{bmatrix}
-14\\-1\\9
\end{bmatrix}
=
\vec{u}\times\vec{v}.
$$

:::

---

## Matrices ortonormales

Sea $Q\in\mathbb{R}^{n\times n}$.

Decimos que $Q$ es **ortonormal** si

$$
Q^\top Q=QQ^\top=I_n.
$$

Esto implica:

- Las columnas $\{\vec{q}_1,\dots,\vec{q}_n\}$ de $Q$ forman una **base ortonormal** de $\mathbb{R}^n$:

$$
    \vec{q}_i^\top\vec{q}_j=
    \begin{cases}
        1,&\text{si }i=j,\\
        0,&\text{si }i\neq j.
    \end{cases}
$$

- Cada columna tiene norma $1$ y distintas columnas son perpendiculares entre sí; en consecuencia, $|\det(Q)|=1$.
- Consecuencia algebraica: $Q^{-1}=Q^\top$.

---

## Valores y vectores propios

Sea $A\in\mathbb{R}^{n\times n}$.

Un escalar $\lambda\in\mathbb{R}$ es un **valor propio** de $A$ si existe un vector no nulo $\vec{v}\in\mathbb{R}^n$ tal que

$$
A\vec{v}=\lambda\vec{v}.
$$

**Condición equivalente:**

$$
\det(\lambda I_n-A)=0.
$$

donde $\det(\lambda I_n-A)$ es el polinomio característico de $A$.


::: {.fragment}

Si $\lambda$ es un valor propio de $A$, entonces todo vector no nulo $\vec{v}\in\mathbb{R}^n$ que cumple

$$
A\vec{v}=\lambda\vec{v}
$$

se denomina **vector propio** asociado a $\lambda$.

El conjunto de todos los vectores propios asociados a un mismo $\lambda$, junto con el vector cero, forma el **espacio propio**:

$$
E_\lambda=\ker(A-\lambda I_n).
$$

:::


---

## Ejemplo: valores y vectores propios

Sea

$$
A=
\begin{bmatrix}
2&0\\
0&3
\end{bmatrix}.
$$

Como

$$
\det(\lambda I-A)=
\begin{vmatrix}
\lambda-2&0\\
0&\lambda-3
\end{vmatrix}
=(\lambda-2)(\lambda-3),
$$

los valores propios son

$$
\lambda_1=2,\qquad \lambda_2=3.
$$

::: {.fragment}

Para $\lambda_1=2$,

$$
(2I-A)\vec{v}=0
\quad\Longrightarrow\quad
\vec{v}=
\alpha
\begin{bmatrix}
1\\0
\end{bmatrix},
\qquad
E_2=\mathrm{span}\left\{
\begin{bmatrix}
1\\0
\end{bmatrix}
\right\}.
$$

Para $\lambda_2=3$,

$$
(3I-A)\vec{v}=0
\quad\Longrightarrow\quad
\vec{v}=
\beta
\begin{bmatrix}
0\\1
\end{bmatrix},
\qquad
E_3=\mathrm{span}\left\{
\begin{bmatrix}
0\\1
\end{bmatrix}
\right\}.
$$

:::

---

## Marco de referencia

El **marco de referencia** $i$ en $\mathbb{R}^3$ es la tupla

$$
\ls{i}{\mathcal{F}}=\left(\ls{i}{O},\;\ls{i}{\mathcal{B}}\right),
$$

donde:

- $\ls{i}{O}\in\mathbb{R}^3$ es el **origen**.
- $\ls{i}{\mathcal{B}}=\{\ls{i}{\vec{x}},\ls{i}{\vec{y}},\ls{i}{\vec{z}}\}$ es una **base ortonormal derecha** de $\mathbb{R}^3$:

$$
    \ls{i}{\vec{x}}\cdot\ls{i}{\vec{y}}=0,
    \qquad
    \|\ls{i}{\vec{x}}\|=\|\ls{i}{\vec{y}}\|=\|\ls{i}{\vec{z}}\|=1,
    \qquad
    \ls{i}{\vec{x}}\times\ls{i}{\vec{y}}=\ls{i}{\vec{z}}.
$$



::: {.fragment}

**Equivalente matricial:**

Si

$$
R=\left[\,\ls{i}{\vec{x}}\;\;\ls{i}{\vec{y}}\;\;\ls{i}{\vec{z}}\,\right],
$$

entonces

$$
    R^\top R=I_3,
    \qquad
    \det(R)=1.
$$


:::


## Marcos de referencia para satélites


::: {.columns}

::: {.column width=50%}

- Un marco de referencia provee un sistema cartesiano para expresar coordenadas de vectores en el espacio. Mediante estos, es posible describir la posición de un cuerpo.

- En general, es necesario definir:

  - Un **marco inercial**.
  - Un **marco sujeto al cuerpo**.
  - Marcos **auxiliares**.


:::

::: {.column width=50%}

![](/Figures/c2/frame_empty.svg){width=100% fig-align="center"}

:::

:::

## Marco de referencia inercial $\ls{i}{\mathcal{F}}$

::: {.columns}

::: {.column}

**Origen:** $\ls{i}{O}$, punto de referencia elegido para describir el movimiento.

**Ejes:**

- $\ls{i}{\vec{x}}$: eje de referencia elegido.
- $\ls{i}{\vec{y}}$: eje perpendicular que completa la base.
- $\ls{i}{\vec{z}}$: eje que completa la regla de la mano derecha.

- Un **marco de referencia inercial** es aquel en el que un cuerpo sobre el que no actúa una fuerza neta permanece en reposo o se mueve con **velocidad constante**.

- En un marco inercial, las **leyes de Newton** pueden aplicarse directamente sin necesidad de introducir fuerzas ficticias asociadas al movimiento del propio marco.


::: {.callout-note}

**Algunos de los marcos inerciales más utilizados son:**

- Marco de referencia inercial geocéntrico (ECI, por sus siglas en inglés).
- Marco de referencia celeste internacional (ICRF, por sus siglas en inglés).

:::

::: 

::: {.column}

![](/Figures/c2/frame_i.svg){width=100% fig-align="center"}

:::

:::

---

::: {.center-vertical}

::: {.chapter-title}

Introducción

:::

<div class="chapter-line"></div>

::: {.chapter-subtitle}

Conceptos teóricos

<div class="menu-title" style="display:none;"> ▸ Conceptos teóricos </div>

:::

:::

## Leyes de Newton



:::{.definition-block}

**Primera ley**

Un cuerpo permanece en reposo o en movimiento rectilíneo uniforme cuando la fuerza neta que actúa sobre él es cero.

$$
\sum \vec{f}=\vec{0}
\quad\Longrightarrow\quad
\vec{v}=\mathrm{cte},
$$

donde $\vec{v}$ es la velocidad traslacional del cuerpo.
:::

:::{.definition-block}

**Segunda ley**

La fuerza neta que actúa sobre un cuerpo es igual a la razón de cambio temporal de su momento lineal.

$$
\sum\vec{f}=\frac{d\vec{\rho}}{dt},
$$

donde $\rho=m\vec{v}$ y $m$ es la masa del cuerpo. 

:::

:::{.definition-block}

**Tercera ley**

Cuando un cuerpo ejerce una fuerza sobre otro, este ejerce simultáneamente sobre el primero una fuerza de igual magnitud y dirección, pero en sentido opuesto.

$$
\vec{f}_{12}=-\vec{f}_{21}.
$$

:::



## Ley de gravitación universal


:::{.definition-block}

**Ley de gravitación universal**

Dos cuerpos con masas $m_1$ y $m_2$ se atraen con una fuerza cuya magnitud es proporcional al producto de sus masas e inversamente proporcional al cuadrado de la distancia entre sus centros:

$$
\vec{f}_{12}=-G\frac{m_1m_2}{\|\vec{r}_{12}\|^2}\hat{r}_{12}
$$

donde $G=6.674\times10^{-11}\:\mathrm{N\,m^2/kg^2}$ es la constante de gravitación universal y $\vec{r}_{12}$ es el vector de posición desde el centro del masa del cuerpo 1 hasta el centro de masa del cuerpo 2.

:::

::: {.incremental}

- La ley de gravitación universal constituye la **base de la mecánica orbital** y permite determinar la fuerza que existe entre dos cuerpos debido a su interacción gravitacional.

- La fuerza gravitacional es una **fuerza conservativa** que depende únicamente de la posición relativa $\vec{r}_{12}$, *i.e.*, el trabajo realizado por la fuerza entre dos puntos es independiente de la trayectoria seguida.

:::


::: {.fragment}

- En consecuencia, el trabajo realizado por la fuerza gravitacional a lo largo de una trayectoria cerrada es cero:

$$
\oint \vec{f}_{12}\cdot \text{d}\vec{r}_{12}=0
$$

:::

::: {.fragment}

- La fuerza gravitacional es además una **fuerza central**, ya que actúa siempre a lo largo de la línea que une los centros de los dos cuerpos.

:::


## Momento lineal

:::{.definition-block}

**Momento lineal**

El momento lineal de un cuerpo es una magnitud vectorial definida como el producto de su masa por su velocidad:

$$
\vec{p}=m\vec{v}
$$

:::

::: {.incremental}

- El momento lineal caracteriza el **estado de movimiento traslacional** de un cuerpo (????).

- Para un cuerpo, la segunda ley de Newton enuncia que la variación del momento lineal total está determinada por la **fuerza externa neta**:

    $$
    \frac{d\vec{\rho}}{dt}=\vec{f}_{\mathrm{ext}}
    $$

- Por lo tanto, cuando la fuerza externa neta es cero, el momento lineal total se **conserva**:
    $$
    \vec{f}_{\mathrm{ext}}=\vec{0}
    \quad\Longrightarrow\quad
    \vec{\rho}=\mathrm{cte.}
    $$

:::


## Momento angular

:::{.definition-block}

**Momento angular**

El momento angular de un cuerpo respecto a un punto de referencia es una magnitud vectorial definida como el producto vectorial entre su posición y su momento lineal:

$$
\vec{h}=\vec{r}\times\vec{p}
$$

donde $\vec{r}$ es el vector de posición desde el punto de referencia hasta el centro de masa del cuerpo. 
:::

::: {.incremental}

- El momento angular caracteriza el **estado de movimiento rotacional** de un cuerpo respecto a un punto de referencia.

- Para un cuerpo, la variación del momento angular está determinada por los **par externo neto**:

    $$
    \frac{d\vec{h}}{dt}=\vec{\tau}_{\mathrm{ext}}
    $$


- Por lo tanto, cuando el par externo neto es cero, el momento angular se **conserva**:

    $$
    \vec{\tau}_{\mathrm{ext}}=\vec{0}
    \quad\Longrightarrow\quad
    \vec{h}=\mathrm{cte.}
    $$

:::
